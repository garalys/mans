        ...
        try:
            ...
            if period_type in ["YoY", "WoW"] and time_period and time_period.startswith("W"):
                ...
                if period_type == "WoW":
                    ...
                    base_tech = get_tech_savings_rate(compare_year, base_week_num)
                    compare_tech = get_tech_savings_rate(compare_year, week_num)

                    # Only apply impact if tech rate changed
                    if abs(base_tech - compare_tech) < 1e-9:
                        bridge_df.loc[idx, "tech_impact"] = 0.0
                    else:
                        base_cpkm = metrics.get("base_cpkm", 0.0)
                        bridge_df.loc[idx, "tech_impact"] = (
                            base_cpkm * (compare_tech - base_tech)
                        )
