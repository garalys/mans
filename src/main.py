...
from .op2.op2_quarterly_bridge import create_op2_quarterly_bridge, create_op2_quarterly_country_business_bridge
from .op2.op2_eu_aggregation import (
    ...
    create_op2_eu_quarterly_business_bridge,
)
...
def run_analytics_pipeline(
    ...
    df = df[df["report_week"].isin(["W05", "W06"])]
    ...
    # Step 14: Create OP2 quarterly business bridges (aggregated from monthly)
    logger.info("Creating OP2 quarterly COUNTRY-BUSINESS bridge...")
    start_time = time.time()

    op2_quarterly_country_business_df = create_op2_quarterly_country_business_bridge(
        df=df,
        df_op2=df_op2,
        df_carrier=df_carrier,
        final_bridge_df=final_bridge_df,
    ...
    logger.info(f"OP2 quarterly COUNTRY-BUSINESS rows: {len(op2_quarterly_country_business_df)}")
    final_bridge_df = pd.concat([final_bridge_df, op2_quarterly_country_business_df], ignore_index=True)
    logger.info(f"OP2 quarterly COUNTRY-BUSINESS creation took: {time.time() - start_time:.2f} seconds")

    logger.info("Creating OP2 EU-BUSINESS quarterly bridge...")
    start_time = time.time()

    op2_eu_quarterly_business_df = create_op2_eu_quarterly_business_bridge(
        df=df,
        df_op2=df_op2,
        df_carrier=df_carrier,
        final_bridge_df=final_bridge_df,
    ...
    logger.info(f"OP2 EU-BUSINESS quarterly rows: {len(op2_eu_quarterly_business_df)}")
    final_bridge_df = pd.concat([final_bridge_df, op2_eu_quarterly_business_df], ignore_index=True)
    logger.info(f"OP2 EU-BUSINESS quarterly creation took: {time.time() - start_time:.2f} seconds")
