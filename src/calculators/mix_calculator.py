...
def compute_hierarchical_mix(
    ...
    logger.info("Country Mix Percentages:")
    for _, row in country_totals.iterrows():
        logger.info(
            f"Country={row['orig_country']} | "
            f"Total Distance={row['country_total']:.0f} | "
            f"Mix={row['country_mix_pct']:.4f}"
        )

    logger.info(
        f"Country mix sum check: {country_totals['country_mix_pct'].sum():.6f}"
    )
